...
class WordPieceTokenizer(object):
    def __init__(self, codes_file, separator='@@', vocab=None, entity_str='', max_entities=500):
        # read/write files as UTF-8
        self.glossaries = ['%s%d_' % (entity_str, i) for i in range(max_entities)]
        self.codes = codecs.open(codes_file, encoding='utf-8')
        self.bpe = BPE(self.codes, separator, vocab, self.glossaries)
        self.entity_str = entity_str

    def tokenize(self, input_str):
        tokens = self.bpe.segment(input_str)
        tokens = [t.replace('@@', '') if t.startswith(self.entity_str) else t for t in tokens]
        return tokens
