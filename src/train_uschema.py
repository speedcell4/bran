...
from src.models.uschema_model import *
...
from src.evaluation.fb15k_eval import *
...
def nearest_neighbors(batcher, sess, model, string_int_maps, batch_k=2):
    print('\nNearest Neighbors')
    ...
    for i in range(batch_k):
        feed_dict, batch_size, doc_ids = uschema_feed_dict(batcher, sess, model,
                                                           FLAGS, string_int_maps=string_int_maps, evaluate=True)
        embeddings, text = sess.run([model.sentence_embeddings, model.text_batch], feed_dict=feed_dict)
        all_embeddings.append(embeddings)
        all_text.append(text)

    flat_embeddings = np.array([x for sublist in all_embeddings for x in sublist])
    flat_text = [x for sublist in all_text for x in sublist]
    text_converted = [' '.join([string_int_maps['token_id_str_map'][token_id] for token_id in seq
                      if token_id != string_int_maps['token_str_id_map']['<PAD>']])
                      for seq in flat_text]

    pair_scores = cosine_similarity(flat_embeddings, flat_embeddings)
    print(flat_embeddings.shape, pair_scores.shape)

    arg_mins = np.argsort(pair_scores, axis=1)

    for i in range(query_k):
        row_idx = random.randint(0, pair_scores.shape[0])

        print('\n')
        print(row_idx, pair_scores[row_idx,row_idx])
        print(text_converted[row_idx])
        print('--------------------------------')
        for j in range(neighbor_k):
            col_idx = arg_mins[row_idx, pair_scores.shape[1]-j-1]
            print(pair_scores[row_idx, col_idx])
            print(text_converted[col_idx])
...
def train_model(model, pos_dist_supervision_batcher, neg_dist_supervision_batcher,
    ...
    while not sv.should_stop() and (max_steps <= 0 or step < max_steps) and (decrease_epochs <= max_decrease_epochs):
            ...
            if step > 0 and step % eval_every == 0:
                if FLAGS.text_prob > 0:
                    nearest_neighbors(positive_train_batcher, sess, model, string_int_maps)
                mrr, hitsAt10 = fb15k_eval.eval()
                print('MRR: %2.2f   Hits@10: %2.2f' % (mrr, hitsAt10))
            ...
            # text update
            if positive_train_batcher is not None and \
                (pos_dist_supervision_batcher is None or (step > kb_pretrain and random.uniform(0, 1) <= text_prob)):
                batcher = positive_train_batcher
                text_update = True
            elif pos_dist_supervision_batcher is not None:
                batcher = pos_dist_supervision_batcher
                text_update = False
            else:
                print('positive_dist_train or positive_train must be provided')
                sys.exit(1)

            feed_dict, batch_size, doc_ids = uschema_feed_dict(batcher, sess, model,
                                                   FLAGS, string_int_maps=string_int_maps, text_update=text_update)
...
def main(argv):
    ...
    with tf.Graph().as_default():
        ...
        ner_test_batcher = NERInMemoryBatcher(FLAGS.ner_test, 1, FLAGS.max_seq, 10) \
            if FLAGS.ner_test else None
        ...
        with sv.managed_session(FLAGS.master,
            ...
            fb15k_eval = None if FLAGS.fb15k_dir == '' else FB15KEvaluator(model, FLAGS.fb15k_dir, string_int_maps, sess)
            tac_eval = None
